 ...
 *
 */
public class FindMaxInIdentifiedRotatedArray {

	public FindMaxInIdentifiedRotatedArray() {
	}
	...
	public int find(int from, int to) {
		...
		while (from <= to) {
			if (list.get(mid) > list.get(mid - 1) && list.get(mid) > list.get(mid + 1)) {
				// 如果中点节点比两个的节点都大 则中点节点为最大值 目标已找到
				return mid;
	...
	public static void main(String[] args) {
		FindMaxInIdentifiedRotatedArray ra = new FindMaxInIdentifiedRotatedArray();
		int size = 10;
